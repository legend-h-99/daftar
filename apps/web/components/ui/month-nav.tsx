...
export function MonthNav({ month, isCurrentMonth, onPrev, onNext, className }: MonthNavProps) {
  ...
  return (
    <div className={`flex items-center justify-between rounded-2xl border border-gray-100 bg-white px-3 py-2.5 shadow-sm ${className ?? ""}`}>
      <button
        type="button"
        onClick={onPrev}
      ...
      <span className="text-sm font-bold text-gray-800">{formatMonthLabel(month, language)}</span>
      <button
        type="button"
        onClick={onNext}
